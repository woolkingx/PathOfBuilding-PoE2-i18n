...
function PassiveSpecListClass:RenameSpec(spec, popupTitle, addOnName)
	local controls = { }
	controls.label = new("LabelControl", nil, {0, 20, 0, 16}, tr("^7Enter name for this passive tree:"))
	...
	main:OpenPopup(370, 100, tr(popupTitle), controls, "save", "edit")
end
...
function PassiveSpecListClass:GetRowValue(column, index, spec)
	if column == 1 then
		...
		return index == self.treeTab.activeSpec and formatUI("%s  ^9(Current)", row) or row
	end
end
...
function PassiveSpecListClass:OnSelDelete(index, spec)
	if #self.list > 1 then
		main:OpenConfirmPopup(tr("Delete Tree"), formatUI("Are you sure you want to delete '%s'?", spec.title or "Default"), tr("Delete"), function()

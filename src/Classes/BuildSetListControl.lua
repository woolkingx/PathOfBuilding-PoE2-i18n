...
local BuildSetListClass = newClass("BuildSetListControl", "ListControl", function(self, anchor, rect, buildMode)
	...
	self.controls.new = new("ButtonControl", { "BOTTOMLEFT", self, "TOP" }, { -190, -4, 60, 18 }, tr("New"),
		function()
			self:NewLoadout()
		end)
	self.controls.rename = new("ButtonControl", { "LEFT", self.controls.new, "RIGHT" }, { 5, 0, 60, 18 }, tr("Rename"),
		function()
	...
	self.controls.copy = new("ButtonControl", { "LEFT", self.controls.rename, "RIGHT" }, { 5, 0, 60, 18 }, tr("Copy"),
		function()
	...
	self.controls.delete = new("ButtonControl", { "LEFT", self.controls.copy, "RIGHT" }, { 5, 0, 60, 18 }, tr("Delete"),
		function()
...
function BuildSetListClass:CustomLoadout(build)
	...
	controls.label = new("LabelControl", nil, { 0, 20, 0, 16 }, tr("^7Enter name for this loadout:"))
	...
	controls.configLabel = new("LabelControl", { "BOTTOMLEFT", controls.configDropDown, "TOPLEFT" }, { 4, -4, 0, 16 },
		tr("^7Copy from Config Set:"))

	controls.save = new("ButtonControl", nil, { -45, 270, 80, 20 }, tr("Save"), function()
	...
	controls.cancel = new("ButtonControl", nil, { 45, 270, 80, 20 }, tr("Cancel"), function()
		main:ClosePopup()
	end)
	main:OpenPopup(370, 300, tr("Create Custom Loadout"), controls, "save", "edit", "cancel")
end

function BuildSetListClass:NewLoadout()
	self:BasicLoadoutPopup({
		popupTitle = tr("New Loadout"),
...
function BuildSetListClass:GetRowValue(column, index, spec)
	if column == 1 then
		...
		local treeLabel = (spec.treeVersion ~= latestTreeVersion and ("[" .. treeVersions[spec.treeVersion].display .. "] ") or "") .. (spec.title or "Default")
		local classLabel = spec.curAscendClassName ~= "None" and spec.curAscendClassName or spec.curClassName
		local row = formatUI("%s (%s, %d points)", treeLabel, classLabel, used)
		return index == self.buildMode.activeLoadout and formatUI("%s  ^9(Current)", row) or row
	end
end
...
function BuildSetListClass:DeleteLoadout(index, spec)
	if #self.list > 1 then
		main:OpenConfirmPopup(tr("Delete Loadout"), formatUI("Are you sure you want to delete '%s'?", spec.title or "Default"),
			tr("Delete"), function()
...
function BuildSetListClass:BasicLoadoutPopup(options)
	...
	controls.label = new("LabelControl", nil, { 0, 20, 0, 16 },
		tr("^7Enter name for this loadout:"))

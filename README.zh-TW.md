# Path of Building 2 — i18n 多語言機制

[English README](README.md)

這個 fork 為
[Path of Building 2 Community](https://github.com/PathOfBuildingCommunity/PathOfBuilding-PoE2)
加上一套 **i18n（多語言）機制**，直接做進引擎裡，而不是從外面套漢化。PoB2 是
流亡黯道 2（Path of Exile 2）最多人用的配裝規劃工具。

**這個專案的重點是機制本身，不是任何單一語言。** 繁體中文（`zh_TW`）只是第一個
參考實作，用來證明這套機制能完整運作。

- **繁體中文（`zh_TW`）** — 完整翻譯，作為參考實作。
...
進去**，讓程式能在顯示層載入翻譯文字。因為機制做在引擎裡，它還能支援外掛式漢化
做不到的事：直接輸入中文（CJK）、用中文搜尋。一旦這個機制存在，翻譯本身就變成
單純的改文字工作，**任何用戶都能做**，任何語言都行，完全不用寫程式。
...
## 給想翻譯任何語言的人：怎麼新增或更新翻譯

所有文字都放在純文字的 **PO 檔**（標準翻譯格式）裡。你不需要懂 Lua 或程式內部，
只要會改文字就能參與，任何語言都適用——繁體中文只是現成的範本。

### 文字放在哪

```text
locale/zh_TW/LC_MESSAGES/    繁體中文（參考實作）
...
要新增一種語言，把 `zh_TW` 的 `.po` 複製到你的語言代碼（例如 `ko`、`ja`）再翻。

### 怎麼改翻譯

1. 用任何文字編輯器（或 Poedit 這類 PO 專用編輯器）打開要改的 `.po` 檔。
2. 找到 `msgid` 底下的英文原文，把 `msgstr` 底下的譯文改成你要的：
...
request。只改翻譯的 PR 很歡迎。維護者主要負責 **i18n 機制**（顯示、輸入、搜尋），
翻譯內容最好由各語言的譯者自己擁有。
...
看第二個提交就能看到相對乾淨原版的完整 i18n diff。補丁只動顯示層；配裝資料、
...
上游擁有 Path of Building 2 Community；本專案是非官方的 i18n fork。

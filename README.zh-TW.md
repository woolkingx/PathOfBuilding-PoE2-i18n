...
進去**，而且涵蓋一個真正的中文化需要的三個面向：

- **顯示**——介面、物品、技能、天賦、屬性說明都顯示成中文。
- **輸入**——輸入框經過修改，可以**直接打中文（CJK）**進搜尋框、名稱框；
  原本的底層引擎根本無法接受中文輸入。
- **搜尋**——搜尋也會比對翻譯後的文字，所以你可以用中文名稱找到物品或技能。

一旦這個機制存在，翻譯本身就變成單純的改文字工作，**任何用戶都能做**，任何語言
都行，完全不用寫程式。
...
中文化是**只動語言、不動資料**：介面、物品、技能、天賦、屬性說明都顯示成中文
（而且可以用中文輸入、用中文搜尋），但你的配裝、匯入/匯出代碼、交易資料維持原本
的英文，這樣才能跟原版 PoB 完全相容、互通。
